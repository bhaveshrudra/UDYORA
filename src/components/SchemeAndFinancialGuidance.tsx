    ...
    <div className="bg-white border border-slate-200/90 rounded-3xl p-5 sm:p-7 shadow-xs space-y-6">
              ...
              >
                ...
                <div>
                  ...
                  <span className={`text-[10px] font-mono block ${isSelected ? 'text-emerald-300' : 'text-slate-500'}`}>
                    {match.matchScore}% Match • {match.qualificationStatus}
                  </span>
                </div>
              </button>
            );
      ...
      <div className="bg-slate-50/80 border border-slate-200/90 rounded-2xl p-4 sm:p-6 space-y-5">
        ...
        <div className="flex flex-col sm:flex-row sm:items-start justify-between gap-4 border-b border-slate-200/80 pb-4">
          <div className="space-y-1.5">
            ...
            <div className="flex flex-wrap items-center gap-2 text-[11px] text-slate-600">
              <span className="font-mono bg-white border border-slate-200 px-2 py-0.5 rounded font-bold">
                Nodal: {s.nodalAgency}
              </span>
              <span>•</span>
              <span>Last Verified: <strong className="font-mono">{s.lastVerifiedDate}</strong></span>
            </div>
          </div>
        ...
        <div className="flex flex-wrap gap-1.5 border-b border-slate-200/80 pb-2">
          {[
            { id: 'financials', label: 'Financial Overview & EMI', icon: IndianRupee },
            { id: 'overview', label: 'Why It Matches', icon: Sparkles },
            { id: 'eligibility', label: 'Eligibility Matrix', icon: CheckCircle2 },
            { id: 'documents', label: 'Document Checklist', icon: FileText },
            { id: 'process', label: 'Application Roadmap (7 Steps)', icon: Layers },
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
              <div className="bg-white border border-blue-200/90 rounded-2xl p-4 space-y-2 shadow-2xs">
                ...
                <p className="text-[11px] text-slate-600 leading-relaxed">
                  Maximum theoretical borrowing limit supported by your own capital of <strong>₹{ownCapital.toLocaleString('en-IN')}</strong> under the scheme's {s.minMarginContributionPct}% promoter equity rule.
                </p>
              </div>
            ...
            <table className="w-full text-left text-xs border-collapse">
              <thead>
                <tr className="bg-slate-100/70 border-b border-slate-200 text-slate-600 font-bold uppercase text-[10px] tracking-wider">
                  <th className="p-3">Eligibility Condition</th>
                  <th className="p-3">Official Requirement</th>
                  <th className="p-3">Your Declared Value</th>
        ...
        {activeSubTab === 'documents' && (
          <div className="space-y-4">
            <div className="flex items-center justify-between text-xs font-bold text-slate-800">
              <span>Interactive Application Readiness Checklist</span>
              <span className="text-[10px] font-mono text-slate-500">
                {Object.values(checkedDocs).filter(Boolean).length} Marked Ready
              </span>
            </div>
        ...
        {activeSubTab === 'process' && (
          <div className="space-y-3">
            <h4 className="text-xs font-black text-slate-950 uppercase tracking-wider">
              Step-by-Step Government Scheme Application Roadmap
            </h4>
              ...
              {(activeSchemeMatch.applicationSteps || []).map((step) => (
                ...
                >
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-3 gap-2 pl-7 pt-1 text-[11px] text-slate-500 border-t border-slate-100">
                    <div><span className="font-bold text-slate-700">Needed:</span> {step.whatIsNeeded}</div>
                    <div><span className="font-bold text-slate-700">Handler:</span> {step.whoHandlesIt}</div>
                    <div><span className="font-bold text-slate-700">Next:</span> {step.whatComesNext}</div>
                  </div>
                </div>
              ))}
            </div>
          ...
          <div className="space-y-3">
            <div className="bg-white border border-slate-200 rounded-2xl p-4 space-y-2 text-xs">
              ...
              <div className="flex items-center gap-4 text-[11px] text-slate-500 pt-2 border-t border-slate-100">
                ...
                <span>Audit Stamp: <strong className="font-mono">{s.lastVerifiedDate}</strong></span>
              </div>
            </div>
          </div>
        )}
